...
> **⚠ Incomplete output detected.** The following sections were not generated...
...
1. **Check token usage.** If a `.checkpoint.json` file exists in `output/`, look at the `tokens_out` value for the affected agent. If it is close to the `max_tokens` limit (currently 32,768 for the content agents, set by `_LARGE_MAX_TOKENS` in `crew.py`), truncation is the likely cause.
